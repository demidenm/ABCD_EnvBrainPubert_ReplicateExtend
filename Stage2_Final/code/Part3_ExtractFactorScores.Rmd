...
abcd = read_csv("./data/ABCD_Updated_2022_04_15.csv")
...
Inverse the family factor score. In the pre-registrationa  the more complex function, `invert` was used. But given that we were only interested in changing the *direction* of the values, we used a simpler version here.
```{r}
...
```{r warning=FALSE}
write.csv(x = abcd_2, file = "./data/ABCD_wFactorScores_2022_04_15.csv")
```
