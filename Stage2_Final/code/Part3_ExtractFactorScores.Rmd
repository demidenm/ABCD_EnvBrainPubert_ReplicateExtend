...
abcd = read_csv("./ABCD_Updated_2022_07_11.csv")
...
Inverse the family factor score. In the pre-registration  the more complex function, `invert` was used. But given that we were only interested in changing the *direction* of the values, we used a simpler version here.
```{r}
#invert <- function(x){ ((x - max(x)) * -1) + min(x) }

abcd_2$FamEnv_Score_r  <- abcd_2$FamEnv_Score * -1
abcd_2$Par_Score_r  <- abcd_2$Par_Score * -1
abcd_2$Demo_Score_r  <- abcd_2$Demo_Score * -1
...
```{r}
Factors = data.frame("Child_CFA" = abcd_2$Child_Score, 
                     "Parent_CFA" = abcd_2$Par_Score,
                     "Parent_CFA_inv" = abcd_2$Par_Score_r, 
                  "Demo_CFA" = abcd_2$Demo_Score, 
                  "Demo_CFA_inv" = abcd_2$Demo_Score_r, 
                  "Fam_Env" = abcd_2$FamEnv_Score, 
                  "Fam_Env_inv" = abcd_2$FamEnv_Score_r,
                  "Par_Monitor"= abcd_2$PMON_y_mean, 
                  "CRPBI" = abcd_2$CRPBI_mean, 
                  "FES_parent" = abcd_2$FES_p_mean_r, "FES_youth" = abcd_2$FES_y_mean_r, 
...
```{r}
Factors %>% 
  pivot_longer(c(Child_CFA, Parent_CFA, Parent_CFA_inv, 
                 Demo_CFA, Demo_CFA_inv, 
                 Fam_Env, Fam_Env_inv), 
...
```{r warning=FALSE}
write.csv(x = abcd_2, file = "./ABCD_wFactorScores_2022_07_11.csv")
```
