...
```{r message=FALSE, warning=FALSE}
data <- read.csv("./data/ABCD_wFactorScores_2022_04_15.csv")
```
...
data <- data %>% 
  rename(
      ...
      FES_youth = FES_y_mean, CRPBI = CRPBI_mean, PMON = PMON_y_mean,
      FES_parent = FES_p_mean,
      # outcome brain
...
For purposes of coherence in abbreviations as we report in the manuscripte, we removed underscores (e.g., FamEnv_Fact --> FamEnv Fact) and renamed some variables as follows.
...
mediation_multiverse <- read.csv("./Output/Multiverse_Table_April_2022.csv") 
