...
abcd = read_csv("./ABCD_cooked_2022_03_22.csv")
...
* FES youth (Higher scores indicate more *negative* aspects of environment)
* FES parent (Higher scores indicate more *negative* aspects of environment)
* CRPBI youth (Higher scores indicate more *positive* aspects of environment)
* Parental monitoring ((Higher scores indicate more *positive* aspects of environment))
...
For purposes of the multiverse analyses, to have interpretation of the mean scores of FES, CRPBI and Parental Monitoring be in the same direction (e.g., higher scores == better environment) we reverse code the means scores of the FES items.
```{r}
# run inverse function on FES youth + parent
abcd_3$FES_y_mean_r <- -1 * abcd_3$FES_y_mean
abcd_3$FES_p_mean_r <- -1 * abcd_3$FES_p_mean

# Compare old to new: FES direction in corr should be flipped, but magnitude should remain the same
Orig_means <- abcd_3 %>%  select(FES_p_mean, FES_y_mean, CRPBI_mean, PMON_y_mean)
New_means <- abcd_3 %>%  select(FES_p_mean_r, FES_y_mean_r, CRPBI_mean, PMON_y_mean)

round(cor(Orig_means,use = "complete.obs",method = "pearson"), 3)
round(cor(New_means,use = "complete.obs",method = "pearson"), 3)
```
...
write.csv(x = abcd_3, file = "./ABCD_Updated_2022_07_11.csv")
```
...
```{r message=FALSE, warning=FALSE}
#jpeg("Figures/Descriptives/VariableDistributions.jpeg", units = "in",
#     width = 8, height = 8, res = 300)

abcd_3 %>% 
  rename("L_AmygCON" = Lamyg_CingOperc, "R_AmygCON" = Ramyg_CingOperc) %>% 
  pivot_longer(c(Age, Par_Edu, Par_Inc,
                 p_puberty, y_puberty, ASRS_t,
                 FES_y_mean_r, FES_p_mean_r, CRPBI_mean, PMON_y_mean,
  ...
  labs(x = "Scales", y = "Score Distributions", 
       caption = "_y = youth, _p = parental; FES (reverse coded), CRPBI, PMON are averages of items.") +
  theme_minimal()
